...
describe 'apache_spark::spark-install' do
  context 'with default settings' do
    ...
    it 'gets the spark 1.2.1 binary' do
      expect(chef_run).to create_remote_file("#{Chef::Config[:file_cache_path]}/spark-1.2.1-bin-hadoop2.4.tgz")
    end

    it 'extracts the tar spark tar file' do
      expect(chef_run).to create_remote_file("#{Chef::Config[:file_cache_path]}/spark-1.2.1-bin-hadoop2.4.tgz")
    end
